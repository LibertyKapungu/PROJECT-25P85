...
def mband(filename, outfile, Nband, Freq_spacing):
    ...
    FRMSZ = 20
    OVLP = 50
    Noisefr = 6  # Matlab recommends 6 but doing 1 so less latency  
    FLOOR = 0.002
    VAD = 1
    ...
    # ==========Start Processing =======
    if AVRGING:
        # Smooth the input spectrum (causal IIR filter)
        filtb = [0.9, 0.1]
        ...
        # Weighted spectral estimate - MATLAB overwrites in-place!
        Wn2, Wn1, W0, W1, W2 = 0.09, 0.25, 0.32, 0.25, 0.09
        
        # NO .copy() - directly overwrite like MATLAB
        ...
        x_magsm[:, nframes - 1] = (Wn2 * x_magsm[:, nframes - 3] + Wn1 * x_magsm[:, nframes - 2] + 
                                    W0 * x_magsm[:, nframes - 1])
        x_magsm[:, nframes - 2] = (Wn2 * x_magsm[:, nframes - 4] + Wn1 * x_magsm[:, nframes - 3] + 
    ...
    # Match MATLAB's overlap-add exactly
    out = np.zeros((nframes - 1) * cmmnlen + frmelen)
    out[:frmelen] = y1_r[:frmelen, 0]  # First frame directly

    start = frmelen - ovlplen
    for i in range(1, nframes):
        mid = start + ovlplen - 1
        stop = start + frmelen - 1
        
        # Overlap region: add
        out[start:mid+1] += y1_r[:ovlplen, i]
        
        # Non-overlap region: direct assignment
        out[mid+1:stop+1] = y1_r[ovlplen:frmelen, i]
