...
if AVRGING
    % smooth the input spectrum (this first pass is already causal)
    filtb = [0.9 0.1];
    x_magsm(:,1) = filter(filtb, 1, x_mag(:,1));
    for i=2:nframes
        x_tmp1 = [x_mag(frmelen-ovlplen,i-1); x_mag(:,i)];
    ...
    % =================== CAUSAL REPLACEMENT START ===================
    % This block replaces the original non-causal weighted spectral
    % estimate with a purely causal version.

    % Define causal weights (for past and current frames only)
    Wn2=0.09; Wn1=0.25; W0=0.32;

    % Renormalize weights since we are only using past/present frames
    total_causal = Wn2 + Wn1 + W0;

    % Frame 1 is left as is after the first-pass filter because there is
    % not enough history for the weighted average.

    % Frame 2 is a special boundary case. It can only be smoothed using
    % frame 1 and itself. We use a smaller, renormalized 2-tap filter.
    if nframes > 1
        temp_total_2 = Wn1 + W0;
        x_magsm(:,2) = (Wn1/temp_total_2 * x_magsm(:,1) + W0/temp_total_2 * x_magsm(:,2));
    end

    % For all subsequent frames (from frame 3 onwards), we can apply the
    % full 3-tap causal filter. The calculation for frame 'i' now only uses
    % data from frames 'i', 'i-1', and 'i-2'.
    for i=3:nframes
        x_magsm(:,i) = (Wn2/total_causal * x_magsm(:,i-2) + ...
                       Wn1/total_causal * x_magsm(:,i-1) + ...
                       W0/total_causal * x_magsm(:,i));
    end
    % =================== CAUSAL REPLACEMENT END =====================
else
    x_magsm = x_mag;
end
