...
OUTPUT_DIR = Path(f"/home/25p85/Gabi/PROJECT-25P85/results/EXP3/spectral/PARAM_SWEEP3/parameter_sweep_analysis_per_snr_{MODE}")
...
# ====================================
df_snr_success = df[df['Status'] == 'Success'].copy()
print(f"\nAnalyzing {len(df_snr_success)} successful tests...")
...
df_snr_success['PESQ_normalized'] = (df_snr_success['PESQ'] + 0.5) / 5.0
df_snr_success['Weighted_Score'] = 0.5 * df_snr_success['PESQ_normalized'] + 0.5 * df_snr_success['STOI']
...
if SNR_COLUMN not in df_snr_success.columns:
    print(f"FATAL ERROR: SNR column '{SNR_COLUMN}' not found!")
    exit(1)

snr_levels = sorted(df_snr_success[SNR_COLUMN].unique())
...
for snr in snr_levels:
    ...
    df_snr_success = df_snr_success[df_snr_success[SNR_COLUMN] == snr].copy()
    ...
    for metric in metrics_to_rank:
        print(f"\n--- Top 10 by {metric} ---")
        top10 = df_snr_success.nlargest(10, metric)[
    ...
    for param in ['Freq_spacing', 'Nband', 'FRMSZ_ms', 'OVLP']:
        ...
        # Store for CSV
        for val in df_snr_success[param].unique():
            subset = df_snr_success[df_snr_success[param] == val]
    ...
    Total tests: {len(df)}
    Successful tests: {len(df_snr_success)} ({len(df_snr_success)/len(df)*100:.1f}%)
