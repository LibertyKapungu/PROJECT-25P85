...
class MultiSNRAudioEnhancementComparator:
    ...
    def __init__(self, csv_files_dict_template, snr_levels, output_folder=None, experiment_name=None):
        ...
        if len(self.method_names) > 6:
            raise ValueError("Maximum 6 methods can be compared")
    ...
    def _extract_noise_types(self, snr):
        """Extract noise types for a specific SNR level."""
        ...
        def categorize_noise(filename):
            ...
            if any(x in filename_lower for x in ['babble', 'cafeteria']):
                ...
                return 'Street'
            elif any(x in filename_lower for x in ['fan', 'cooler', 'ssn', 'white']):
                return 'Stationary'
            else:
                return 'Other'
            ...
            for i, method_name in enumerate(self.method_names):
                ...
                if snr_values:
                    ax.plot(snr_values, metric_values, 
                           ...
                           label=method_name, color=self.colors[i], alpha=0.8)
    ...
    def plot_snr_noise_category_heatmap(self, metric='PESQ', figsize=(16, 10)):
        ...
        for idx, method_name in enumerate(self.method_names):
            ...
            # Plot heatmap
            im = ax.imshow(data_matrix, cmap='RdYlGn', aspect='auto', vmin=0, vmax=4 if metric == 'PESQ' else 1)
    ...
    def plot_snr_bar_comparison(self, snr, figsize=(20, 12)):
        """Create bar chart for a specific SNR level (same as original)."""
    ...
    def export_all_results(self):
        ...
        plt.savefig(amalg_plot_path, dpi=300, bbox_inches='tight')
        plt.close(fig_amalg)
...
if __name__ == "__main__":
    ...
    csv_files_template = {
        'Baseline_Noisy': "C:\\Users\\gabi\\Documents\\University\\Uni2025\\Investigation\\PROJECT-25P85\\results\\BASELINE\\NOIZEUS_EARS_BASELINE\\BASELINE_NOIZEUS_EARS_[{snr}]dB.csv",
        'GTCRN_baseline': "C:\\Users\\gabi\\Documents\\University\\Uni2025\\Investigation\\PROJECT-25P85\\results\\EXP3\\EXP3p1a\\GTCRN_NOIZEUS_EARS_[{snr}]dB.csv",
        'SS_GTCRN_n4_ov75_fr8_fl08_v0_lin': "C:\\Users\\gabi\\Documents\\University\\Uni2025\\Investigation\\PROJECT-25P85\\results\\EXP3\\EXP3p1b_GTCRN_SS_pp_8ms_V0_f08\\GTCRN_SS_TEST2_[{snr}]dB.csv",
        'WF_GTCRN_fr25_mu0.98_a_dd0.98_eta0.15': "C:\\Users\\gabi\\Documents\\University\\Uni2025\\Investigation\\PROJECT-25P85\\results\\EXP3\\EXP3p1b_GTCRN_WF_ss\\GTCRN_MWF_merged_[{snr}]dB.csv",
    }
    
    # Set output folder
    output_folder = "C:\\Users\\gabi\\Documents\\University\\Uni2025\\Investigation\\PROJECT-25P85\\results\\compare_csvs\\poster_prep_multi_snr1"
    ...
    comparator = MultiSNRAudioEnhancementComparator(
        ...
        experiment_name="GTCRNs_Multi_SNR"
    )
