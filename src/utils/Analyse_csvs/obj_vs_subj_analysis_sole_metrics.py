...
Additionally, do the same for individual metrics (PESQ, STOI, SI-SDR, DNSMOS)
"""
...
# Functionality as above for individual metrics. pesq, stoi, si-sdr, dnsmos


# ====================================
# NEW SECTION: INDIVIDUAL METRIC PLOTS
# ====================================
print("\n" + "="*100)
...
individual_metrics = {
    'PESQ': {'ylabel': 'PESQ Score', 'title': 'PESQ'},
    'STOI': {'ylabel': 'STOI Score', 'title': 'STOI'},
    'SI_SDR': {'ylabel': 'SI-SDR (dB)', 'title': 'SI-SDR'},
    'DNSMOS_mos_ovr': {'ylabel': 'DNSMOS Score', 'title': 'DNSMOS'}
}


# Combined plots: each parameter gets one figure with 4 subplots (one per metric)
# Include FRMSZ? 
for param in ['FRMSZ_ms', 'Nband', 'OVLP', 'Floor']:
    print(f"\nGenerating combined metric plots for {param}...")
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle(f'{param}: Metrics Across SNR', fontsize=16, fontweight='bold')
    ...
    for i, (metric_name, metric_info) in enumerate(individual_metrics.items()):
        ax = axes[i]
        
        # Plot each parameter value
        for val in param_values:
            subset = df_success[df_success[param] == val]
            snr_avg = subset.groupby('SNR_dB')[metric_name].mean()
        ...
        ax.set_title(metric_info['title'], fontsize=13, fontweight='bold')
        ax.grid(True, alpha=0.3)
        ax.legend(title=param, fontsize=9)
    
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # leave space for suptitle
    filename = f'combined_metrics_by_{param}.png'
    plt.savefig(OUTPUT_DIR / filename, dpi=300, bbox_inches='tight')
    print(f"✓ Saved: {filename}")
    plt.close()

# Generate individual metric heatmaps for Freq_spacing vs SNR
...
for metric_name, metric_info in individual_metrics.items():
    ...
    for snr in snr_levels:
        ...
        for freq in freq_values:
            subset = df_success[(df_success['SNR_dB'] == snr) & (df_success['Freq_spacing'] == freq)]
            if len(subset) > 0:
                row.append(subset[metric_name].mean())
            else:
                row.append(np.nan)
    ...
    ax.set_title(f'{metric_info["title"]} by Freq_spacing and SNR', fontsize=14, fontweight='bold')
    ax.set_xlabel('Frequency Spacing', fontsize=12)
    ax.set_ylabel('SNR Level', fontsize=12)
    
    plt.tight_layout()
    filename = f'heatmap_individual_{metric_name}_freq_spacing.png'
    plt.savefig(OUTPUT_DIR / filename, dpi=300, bbox_inches='tight')
    print(f"  ✓ Saved: {filename}")
    plt.close()

print(f"\n" + "="*100)
print("INDIVIDUAL METRIC VISUALIZATIONS COMPLETE!")
print("="*100)

print(f"\n Additional generated files:")
print(f"  ✓ individual_PESQ_by_*.png (for Freq_spacing, Nband, OVLP)")
print(f"  ✓ individual_STOI_by_*.png (for Freq_spacing, Nband, OVLP)")
print(f"  ✓ individual_SI_SDR_by_*.png (for Freq_spacing, Nband, OVLP)")
print(f"  ✓ individual_DNSMOS_mos_ovr_by_*.png (for Freq_spacing, Nband, OVLP)")
print(f"  ✓ heatmap_individual_PESQ_freq_spacing.png")
print(f"  ✓ heatmap_individual_STOI_freq_spacing.png")
print(f"  ✓ heatmap_individual_SI_SDR_freq_spacing.png")
print(f"  ✓ heatmap_individual_DNSMOS_mos_ovr_freq_spacing.png")
print("="*100 + "\n")