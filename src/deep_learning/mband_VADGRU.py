...
def noiseupdt(x_magsm, n_spect, hop, n_frames):
    """Voice Activity Detection and noise spectrum update"""
    SPEECH = 1
    SILENCE = 0
    state = np.zeros(n_frames * hop, dtype=int)
    judgevalue1 = np.zeros(n_frames * hop)
    ...
    judgevalue1[i*hop:(i+1)*hop] = judgevalue
    
    if judgevalue > 0.4:  # Different threshold for first frame
        state[i*hop:(i+1)*hop] = SPEECH
    else:
        state[i*hop:(i+1)*hop] = SILENCE
...
def mband(
    ...
    Args:
         ...
         frame_dur_ms - Frame length in milli-seconds, default=8. 
   ...
   Example call:
            enhanced_audio, fs = mband(noisy_audio, fs, output_dir='output/', output_file='enhanced.wav', Nband=4, Freq_spacing='linear', frame_dur_ms=8, OVLP=50, AVRGING=1, Noisefr=1, FLOOR=0.002, VAD=1)
    ...
    input_name = input_name if input_name is not None else "SS_"
    ...
    #     waveform = torch.mean(noisy_audio, dim=0)
    # else:
    #     waveform = noisy_audio.squeeze()
    ...
    # --- Frame / window setup ---
    frame_samples = int(frame_dur_ms * fs / 1000) # Frame size in samples
    # overlap_samples  = int(np.floor(frame_samples * OVLP / 100)) # Number of overlap samples
    # hop = frame_samples - overlap_samples   # Number of common samples between adjacent frames
    ...
    U = np.sum(analysis_win @ analysis_win) / frame_samples  # Normalization factor for overlap-add
    ...
    for k in range(Noisefr):
        n_fft = fft(waveform[j:j + frame_samples]* analysis_win, fftl)
        n_mag = np.abs(n_fft)
        n_ph = np.angle(n_fft)
        n_magsq = n_mag ** 2
        noise_ps += n_magsq
        j += frame_samples
    n_spect = np.sqrt(noise_ps / Noisefr).reshape(-1, 1)
    ...
    sample_pos = 0
    while sample_pos + frame_samples <= len(waveform):
        current_frame = waveform[sample_pos:sample_pos + frame_samples]
        windowed_frame = current_frame * analysis_win
        ...
        # Advance by hop size (hop) for proper overlap
        sample_pos += hop 
        frame_count += 1
    ...
    if AVRGING:             
        ...
        for i in range(1, n_frames):
            x_tmp1 = np.concatenate([x_mag[frame_samples - hop :, i - 1], x_mag[:, i]])
    ...
    if VAD:
        n_spect_expanded = np.tile(n_spect, (1, n_frames))
        n_spect, state = noiseupdt(x_magsm, n_spect_expanded, hop, n_frames)
    else:
    ...
    for i in range(Nband):
        ...
        for j in range(n_frames):
            signal_power = (np.linalg.norm(x_magsm[start:stop, j], 2) ** 2)/(frame_samples*U) # the term frmlem*U may not be neccessary as they cancel out (snr). It only matters if computing absolute power values, not ratios.
            noise_pser = (np.linalg.norm(n_spect[start:stop, j], 2) ** 2)/(frame_samples*U)
            SNR_x[i, j] = 10 * np.log10(signal_power / (noise_pser + 1e-10))
...
def prepare_audio_data(clean_path, noise_path, target_sr, snr_db):
    ...
    waveform = clean + noise_scaled
    
    return waveform, clean, noise_scaled, target_sr
