"""
Experiment GTCRN_EXP3p1a: GTCRN Enhanced Speech Metrics (NOIZEUS + EARS Mixture)

This experiment evaluates GTCRN enhancement performance by:
...
3. Computing quality metrics on the enhanced speech

Processing: Clean speech + noise -> noisy mixture -> GTCRN enhancement -> compute quality metrics
...
results_dir = repo_root / 'results' / 'EXP3' / 'EXP3p1a'
...
gtcrn_model, device = load_gtcrn_model(checkpoint_path, device='cpu')
...
for snr_dB in snr_dB_range:
    ...
    for noise_path, clean_path in paired_files:
        ...
        enhanced_speech = enhance_with_gtcrn(
        ...
        print("4. Computing speech quality metrics...")
        metrics = compute_and_save_speech_metrics(
            clean_tensor=clean_waveform_16k,
            enhanced_tensor=enhanced_speech,
            fs=processing_sr,
            clean_name=clean_filename,
            enhanced_name=f"GTCRN_{clean_filename}_{noise_filename}_SNR[{snr_dB}]dB",
            csv_dir=str(results_dir_snr),
            csv_filename='GTCRN_NOIZEUS_EARS_metrics.csv'
        )
        
        # Print summary
        print(f"\n{'='*100}")
        print(f"Completed GTCRN enhancement for:")
    ...
    merged_path = merge_csvs(
        ...
        output_filename=f'GTCRN_NOIZEUS_EARS_[{snr_dB}]dB.csv',
        keep_source=True
    )
