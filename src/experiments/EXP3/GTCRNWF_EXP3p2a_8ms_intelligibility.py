"""
Experiment GTCRNWF_EXP3p2a_8ms_intelligibility: GTCRN with Wiener Filter Post-processing (8ms frames, intelligibility-optimized)
...
by Wiener filter post-processing optimized for intelligibility (mu=0.123).

Processing pipeline:
1. GTCRN model - removes initial noise and distortions
2. Wiener filter (8ms frames, mu=0.123) - further refines the enhanced speech from GTCRN
...
- 8ms frame duration for Wiener filter post-processing.
- mu=0.123 optimized for intelligibility.
...
artifacts that could affect the neural network's output. This version optimizes for intelligibility.
...
output_dir = repo_root / 'sound_data' / 'processed' / 'gtcrn_processed_outputs' / 'GTCRNWF_EXP3p2a_8ms_intelligibility_output' 
results_dir = repo_root / 'results' / 'EXP3' / 'GTCRN' / 'GTCRNWF_EXP3p2a_8ms_intelligibility'
...
for snr_dB in snr_dB_range:
    ...
    for noise_path, clean_path in paired_files:
        ...
        output_filename = f"GTCRNWF_8ms_intelligibility_{clean_filename}_{noise_filename}_SNR[{snr_dB}]dB.wav"
        ...
        metrics = compute_and_save_speech_metrics(
            ...
            csv_filename='GTCRNWF_EXP3p2a_8ms_intelligibility_data'
        )
    ...
    merged_path = merge_csvs(
        ...
        output_filename=f'GTCRNWF_EXP3p2a_8ms_intelligibility_merged_[{snr_dB}]dB.csv',
        keep_source=True
    )
