"""
Fixed script to run Spectral Subtraction filtering experiment with updated module structure.
"""
...
def main():
    """Run complete Spectral Subtraction Mband filtering experiment."""
    ...
    print(f"Starting Spectral Subtraction Mband filtering experiment:")
    ...
    enhanced_speech, enhanced_fs = mband(
        ...
        output_file=f'spectral_filter_priori_P{participant_id}_S{sentence}_F{fold}_SNR{snr}',
    ...
    generate_and_save_spectrogram(
        ...
        title=f'Spectral Enhanced Speech - P{participant_id} S{sentence} F{fold} SNR{snr}dB',
        include_metadata_in_filename=True,
        audio_name=f'spectral_enhanced_P{participant_id}_S{sentence}_F{fold}_SNR{snr}'
    )
    ...
    metrics = compute_and_save_speech_metrics(
        ...
        csv_filename='spectral_mband_filter_results'
    )
