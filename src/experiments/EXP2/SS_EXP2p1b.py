...
# Checking for difference in VAD GRU for spectral
...
output_dir = repo_root / 'sound_data' / 'processed' / 'spectral_processed_outputs' / 'EXP2_output' 
results_dir = repo_root / 'results' / 'EXP2' / 'spectral' / 'SS_EXP2p1b'
...
from deep_learning.mband_VADGRU import mband
...
for threshold in [threshold]:
    for snr_dB in snr_dB_range:
        ...
        for noise_path, clean_path in paired_files:
            ...
            output_filename = f"SS_{clean_filename}_{noise_filename}_SNR[{snr_dB}]dB_THRESH[{threshold:.2f}].wav"
            ...
            enhanced_speech, enhanced_fs = mband(
                ...
                # use_causal_vad=True,  # Real-time compatible
            )
            ...
            metrics = compute_and_save_speech_metrics(
                ...
                csv_filename=f'SS_EXP2.1b_data_SNR[{snr_dB}]dB_THRESH[{threshold:.2f}]'
            )
...
for snr_dB in snr_dB_range:
    ...
    merged_path = merge_csvs(
        ...
        output_filename=f'SS_EXP1p2_merged_[{snr_dB}]dB.csv',
        keep_source=True
    )
