"""
GTCRN-SS hybrid to create audio
...
results_dir = repo_root / 'sound_data' / 'processed' / 'GTCRN_SS'/ 'GTCRN_SS_dif_wt_deltas'/'log_N16_F20_075_FL0p8'
...
from dsp_algorithms.spectral.mband_no_deltas import mband
...
paired_files = paired_files[19:20]  # Limit to first 2 pairs for testing
...
for snr_dB in snr_dB_range:
    ...
    for noise_path, clean_path in paired_files:
        ...
        # save audio file 
        gtcrn_enhanced = gtcrn_enhanced.unsqueeze(0)

        torchaudio.save(results_dir_snr / f"GTCRN_enhanced_[{snr_dB}]dB.wav", gtcrn_enhanced.cpu(), processing_sr)

        # print metrics for gtcrn only
        print("Computing GTCRN-only speech quality metrics...")
        gtcrn_metrics = compute_and_save_speech_metrics(
            clean_tensor=clean_waveform_16k,
            enhanced_tensor=gtcrn_enhanced,
            fs=processing_sr,
            clean_name=f"{clean_path.parent.name}_{clean_path.stem}",
            enhanced_name=f"GTCRN_only_{clean_path.parent.name}_{clean_path.stem}_{noise_path.parent.name}_{noise_path.stem}_SNR[{snr_dB}]dB",
            csv_dir=str(results_dir_snr),
            csv_filename='GTCRN_only_NOIZEUS_EARS_metrics.csv'
        )
        print(f"GTCRN-only Metrics: PESQ: {gtcrn_metrics['PESQ']:.3f}, STOI: {gtcrn_metrics['STOI']:.3f}, SI-SDR: {gtcrn_metrics['SI_SDR']:.2f} dB, DNSMOS Overall: {gtcrn_metrics['DNSMOS_mos_ovr']:.3f}")
        ...
        final_enhanced_speech, final_fs = mband(
            ...
            fs=processing_sr,
            output_dir=results_dir_snr,
            output_file=f"GTCRN_SS_[{snr_dB}]dB.wav",
            input_name="GTCRN_enhanced_",
            Nband=16,
            Freq_spacing='log',
            FRMSZ=20,
            OVLP=75,
    ...
    merged_path = merge_csvs(
        ...
        output_filename=f'GTCRN_SS_[{snr_dB}]dB.csv',
        keep_source=True
    )
