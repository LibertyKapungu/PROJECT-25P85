...
def noiseupdt(x_magsm, n_spect_initial, cmmnlen, nframes):
    ...
    # Ensure n_spect_initial is 1D
    n_current = n_spect_initial.squeeze() if n_spect_initial.ndim == 2 else n_spect_initial
    ...
    for i in range(nframes):
        ...
        threshold = 0.4 if i == 0 else 0.45

        if judgevalue > threshold:  # Different threshold for subsequent frames
            state[i*cmmnlen:(i+1)*cmmnlen] = SPEECH
            n_updated = n_current.copy()
        else:
            state[i*cmmnlen:(i+1)*cmmnlen] = SILENCE
            n_updated = np.sqrt(0.9 * n_current**2 + 0.1 * x_magsm[:, i]**2)
    
        n_spect_frames.append(n_updated)
        n_current = n_updated

    n_spect = np.array(n_spect_frames).T
...
def mband(
    ...
    # If no complete frames, handle edge case
    if nframes == 0:
        print("Warning: Audio too short for processing")
        return torch.zeros_like(noisy_audio), fs
    ...
    for frame_idx in range(nframes):
        # ===== STEP 1: EXTRACT & FFT CURRENT FRAME (ON-THE-FLY) =====
        ...
        # Advance sample position for next frame
        sample_pos += cmmnlen
        
        # ===== STEP 2: IIR SMOOTHING (CAUSAL) =====
        if AVRGING:
            if frame_idx == 0:
                # First frame: no history
                frame_smoothed = scipy.signal.lfilter(filtb, [1], frame_mag)
            else:
                # Use overlap from previous frame for continuity
        ...
        # ===== STEP 3: VAD & NOISE UPDATE (CAUSAL) =====
        if VAD:
            ...
            is_speech = True  # Assume all speech
            n_updated = n_current.copy()
        
        # ===== STEP 4: WEIGHTED AVERAGING (CAUSAL, ADAPTIVE) =====
        # === BETTER ADAPTIVE WEIGHTS ===
        if AVRGING and frame_idx >= 2:
            # Use SNR instead of binary VAD
            # Calculate broadband SNR for current frame
            ...
            # Smooth weight selection based on SNR
            if frame_snr_db > 10.0:  # High SNR (clean speech)
                Wn2, Wn1, Wn0 = 0.09, 0.25, 0.66  # Responsive
            elif frame_snr_db < 0.0:  # Low SNR (noisy)
                Wn2, Wn1, Wn0 = 0.12, 0.30, 0.58  # Conservative
            else:  # Moderate SNR (0-10 dB)
                # Linear interpolation between conservative and responsive
                alpha = frame_snr_db / 10.0  # 0 to 1
                Wn2 = 0.12 + alpha * (0.09 - 0.12)  # 0.12 → 0.09
                Wn1 = 0.30 + alpha * (0.25 - 0.30)  # 0.30 → 0.25
                Wn0 = 0.58 + alpha * (0.66 - 0.58)  # 0.58 → 0.66
            
            frame_final = (Wn2 * prev_prev_smoothed_mag + 
                        Wn1 * prev_smoothed_mag + 
                        Wn0 * frame_smoothed)
        elif AVRGING and frame_idx == 1:
            # Frame 1: only 2-tap
            frame_final = 0.30 * prev_smoothed_mag + 0.70 * frame_smoothed
        else:
            # Frame 0 or AVRGING=0
            frame_final = frame_smoothed.copy()
        
        # ===== STEP 5: SNR CALCULATION & SPECTRAL SUBTRACTION (PER-BAND) =====
        enhanced_mag_frame = np.zeros(fftl // 2 + 1)
        
        for band_idx in range(Nband):
            if band_idx < Nband - 1:
                start = lobin[band_idx]
                stop = hibin[band_idx] + 1
            else:
                start = lobin[band_idx]
                stop = fftl // 2 + 1
            ...
            # Compute beta (over-subtraction factor)
            ...
            n_spec_sq = n_updated[start:stop] ** 2
            sub_speech = frame_final[start:stop] ** 2 - beta * n_spec_sq * delta
        ...
        # ===== STEP 6: RECONSTRUCT SPECTRUM & IFFT =====
        ...
        # ===== STEP 7: WOLA SYNTHESIS (ACCUMULATE) =====
        start_idx = frame_idx * cmmnlen
        out[start_idx:start_idx + frmelen] += y_frame[:frmelen] * win
        win_sum[start_idx:start_idx + frmelen] += win
