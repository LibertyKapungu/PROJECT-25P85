...
def mband(
   ...
   Example call:
    ...
    frame_samples = int(np.floor(FRMSZ * fs / 1000))  # Frame size in samples
    overla_samples  = int(np.floor(frame_samples * OVLP / 100)) # Number of overlap samples
    hop = frame_samples - overla_samples   # Number of common samples between adjacent frames
    ...
    U = np.sum(analysis_win**2) / frame_samples  # Normalization factor for overlap-add

    # Estimate noise magnitude for first 'Noisefr' frames
    noise_pow = np.zeros(fftl)
    j = 0
    for k in range(Noisefr):
        n_fft = fft(noisy_speech[j:j + frame_samples]* analysis_win, fftl)
        n_mag = np.abs(n_fft)
        n_ph = np.angle(n_fft)
        n_magsq = n_mag ** 2
        noise_pow += n_magsq
    ...
    sample_pos = 0
    while sample_pos + frame_samples <= len(noisy_speech):
        current_frame = noisy_speech[sample_pos:sample_pos + frame_samples]
    ...
    for i in range(Nband):
        ...
        for j in range(n_frames):
            ...
            noise_power = (np.linalg.norm(n_spect[start:stop, j], 2) ** 2)/(frame_samples*U)
