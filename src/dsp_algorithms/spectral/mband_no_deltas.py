...
def mband(
    ...
    # Extract and FFT current frame
        ...
        for band_idx in range(Nband):
            ...
            # Frequency-based delta (matches textbook equation)
            f_upper = hibin[band_idx] * fs / fftl  # Convert bin to frequency

            if f_upper <= 1000:  # 1 kHz
                delta = 1.0
            elif f_upper <= (fs/2 - 2000):  # Fs/2 - 2kHz
                delta = 2.5
            else:
                delta = 1.5
