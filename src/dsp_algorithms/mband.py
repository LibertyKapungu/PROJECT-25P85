...
def berouti(SNR):
    # MATLAB: a(i,j)=4-SNR(i,j)*3/20; or 4.75 or 1
    a = np.where(
        (SNR >= -5.0) & (SNR <= 20),
        4 - SNR * 3 / 20,
        np.where(SNR < -5.0, 4.75, 1)
    )
    return a
...
def estfilt1(nChannels, Srate):
    ...
    UpperFreq = FS
    LowFreq = 1
    range_ = np.log10(UpperFreq / LowFreq)
    interval = range_ / nChannels
    ...
    for i in range(nframes):
        ...
        if (i == 0 and judgevalue > 0.4) or (i > 0 and judgevalue > 0.45):
            ...
            if i > 0:
                ...
                n_spect[:, i] = np.sqrt(0.9 * n_spect[:, 0] ** 2 + 0.1 * x_magsm[:, 0] ** 2)
...
def mband(filename, outfile, Nband, Freq_spacing):
    ...
    for k in range(Noisefr):
        current_frame = noisy_speech[j:j + frmelen]
        if len(current_frame) < frmelen:
            current_frame = np.pad(current_frame, (0, frmelen - len(current_frame)))
        n_fft = fft(current_frame * win, fftl)
        n_mag = np.abs(n_fft)
        n_magsq = n_mag ** 2
        noise_pow += n_magsq
        j += frmelen
    ...
    # Framing
    framed_x = frame(noisy_speech, win, ovlplen)
    ...
    if AVRGING:
        ...
        for i in range(1, nframes):
            ...
            x_magsm[:, i] = x_tmp2[1:1 + x_mag.shape[0]]

        # Weighted spectral estimate (temporal smoothing across frames)
        Wn2, Wn1, W0, W1, W2 = 0.09, 0.25, 0.32, 0.25, 0.09
        # Pad for edge cases
        pad = np.zeros((x_magsm.shape[0], 2))
        x_pad = np.concatenate([pad, x_magsm, pad], axis=1)
        x_smooth = np.zeros_like(x_magsm)
        for i in range(nframes):
            if i == 0:
                x_smooth[:, i] = (W0 * x_pad[:, 2] + W1 * x_pad[:, 3] + W2 * x_pad[:, 4])
            elif i == 1:
                x_smooth[:, i] = (Wn1 * x_pad[:, 2] + W0 * x_pad[:, 3] + W1 * x_pad[:, 4] + W2 * x_pad[:, 5])
            elif 2 <= i <= nframes - 3:
                x_smooth[:, i] = (Wn2 * x_pad[:, i] + Wn1 * x_pad[:, i + 1] + W0 * x_pad[:, i + 2] +
                                  W1 * x_pad[:, i + 3] + W2 * x_pad[:, i + 4])
            elif i == nframes - 2:
                x_smooth[:, i] = (Wn2 * x_pad[:, i] + Wn1 * x_pad[:, i + 1] + W0 * x_pad[:, i + 2] +
                                  W1 * x_pad[:, i + 3])
            elif i == nframes - 1:
    ...
    for i in range(Nband):
        ...
        for j in range(nframes):
            SNR_x[i, j] = 10 * np.log10(
                np.linalg.norm(x_magsm[start:stop, j], 2) ** 2 /
                (np.linalg.norm(n_spect[start:stop, j], 2) ** 2 + 1e-8)
            )
