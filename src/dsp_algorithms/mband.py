...
def mband(filename, outfile, Nband, Freq_spacing):
    ...
    if Freq_spacing.lower() == 'linear':
        ...
        lobin = [i * bandsz[0] + 1 for i in range(Nband)]  # The lower (lobin) and upper (hibin) bin indices for each band are then determined so that each band covers an equal range of frequency bin
