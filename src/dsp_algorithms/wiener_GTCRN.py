...
def wiener_filter(
    ...
    for j in range(nsubframes):
        ...
        noise_power = (X.abs() ** 2) / (frame_samples * U)
        init_frame_powers.append(noise_power)

    init_frame_powers = torch.stack(init_frame_powers, dim=0)
    noise_ps = init_frame_powers.quantile(0.05, dim=0)
