...
def mband(
    ...
    #     state = np.ones(nframes * cmmnlen, dtype=int)  # Assume all speech
    ...
    if VAD:
        ...
        n_spect = np.repeat(n_spect, nframes, axis=1)
        state = np.ones(nframes * cmmnlen, dtype=int)

    # ============ ADAPTIVE WEIGHTED AVERAGING ============
    if AVRGING:
        x_magsm_filtered = x_magsm.copy()
        if nframes > 1:
            x_magsm_filtered[:, 1] = 0.30 * x_magsm[:, 0] + 0.70 * x_magsm[:, 1]
            for i in range(2, nframes):
                frame_start_idx = i * cmmnlen
                frame_end_idx = min((i + 1) * cmmnlen, len(state))
                local_state = state[frame_start_idx:frame_end_idx]
                local_speech_ratio = np.sum(local_state) / len(local_state) if len(local_state) > 0 else 0.5
                
                if local_speech_ratio > 0.65:
                    Wn2, Wn1, Wn0 = 0.09, 0.25, 0.66
                elif local_speech_ratio < 0.45:
                    Wn2, Wn1, Wn0 = 0.12, 0.30, 0.58
                else:
                    Wn2, Wn1, Wn0 = 0.10, 0.28, 0.62
        ...
        assert not np.any(np.isnan(x_magsm)), "NaN detected"
        assert not np.any(np.isinf(x_magsm)), "Inf detected"
        if nframes > 1:
            print(f"Spectral continuity: {np.mean(np.abs(np.diff(x_magsm, axis=1))):.6f}")
    # end of change
